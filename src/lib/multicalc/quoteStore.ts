...
export function generateResults(form: QuoteFormData): Quote[] {
  const baseFactor =
    (form.coberturas.terceiros === "200.000"
      ? 1.15
      : form.coberturas.terceiros === "50.000"
        ? 0.85
        : 1) *
    (form.coberturas.carroReserva === "Sim" ? 1.05 : 1) *
    (form.coberturas.vidros === "Completo"
      ? 1.04
      : form.coberturas.vidros === "Para-brisa"
        ? 1.02
        : 1) *
...
const seedRecords = (): QuoteRecord[] => {
  ...
  recs.push({
    ...
    results: variantQuotes(0),
    winnerInsurer: "SulAmérica",
  });
  recs.push({
    id: "q1-2",
    groupId: g1,
  ...
  recs.push({
    ...
    results: variantQuotes(60, 0.95),
    winnerInsurer: "Porto Seguro",
  });
  ...
  recs.push({
    ...
    results: variantQuotes(-200),
    winnerInsurer: "Bradesco",
  });
  recs.push({
    id: "q2-2",
    groupId: g2,
  ...
  recs.push({
    ...
    results: variantQuotes(80),
    winnerInsurer: "Allianz",
  });
  recs.push({
    id: "q4-2",
    groupId: "g4",
  ...
  recs.push({
    ...
    results: variantQuotes(420),
    winnerInsurer: "Mapfre",
  });
  return recs;
};
...
const FIELD_LABELS: Record<string, string> = {
  ...
  "objeto.tipo": "Tipo",
  "objeto.marcaModelo": "Marca/Modelo",
  "objeto.ano": "Ano",
  "objeto.cep": "CEP",
  "coberturas.terceiros": "Terceiros",
  "coberturas.carroReserva": "Carro reserva",
  "coberturas.vidros": "Vidros",
  "coberturas.assistencia24h": "Assistência 24h",
};
...
export function QuoteStoreProvider({ children }: { children: ReactNode }) {
  ...
  const addNewQuote = useCallback(
    (data: QuoteFormData, results: Quote[], winner: Insurer, createdBy = "Ana Souza") => {
  ...
  const setStatus = useCallback(
    (groupId: string, status: QuoteStatus, lostReason?: LostReason, lostNote?: string) => {
      setRecords((r) =>
        r.map((x) =>
          x.groupId === groupId
            ? {
    ...
    return Array.from(map.entries())
      .map(([groupId, versions]) => {
        ...
        return {
          ...
          versions,
          latest,
          status: effectiveStatus(latest, versions),
        };
      })
