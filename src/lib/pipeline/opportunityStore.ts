 ...
 */
function buildSyntheticStageHistory(
  current: KanbanStage,
  createdAt: string,
  closedAt?: string,
): StageHistoryEntry[] {
...
export function PipelineStoreProvider({ children }: { children: ReactNode }) {
  ...
  const moveStage = useCallback(
    (id: string, stage: KanbanStage, lostReason?: LostReason, lostNote?: string) => {
  ...
  const linkQuoteGroup = useCallback((opportunityId: string, quoteGroupId: string) => {
    setOpportunities((arr) =>
      arr.map((t) => (t.id === opportunityId ? { ...t, quoteGroupId } : t)),
    );
  }, []);
  ...
  const unlinkQuoteGroup = useCallback((quoteGroupId: string) => {
    setOpportunities((arr) =>
      arr.map((t) => (t.quoteGroupId === quoteGroupId ? { ...t, quoteGroupId: undefined } : t)),
    );
  }, []);
  ...
  const removeCommentAttachment = useCallback(
    (id: string, commentId: string, attachmentId: string) => {
      setOpportunities((arr) =>
        arr.map((t) => {
          if (t.id !== id) return t;
          const c = t.comments.find((x) => x.id === commentId);
          ...
          return {
            ...
            comments: t.comments.map((x) =>
              x.id === commentId
                ? {
                    ...x,
                    attachmentIds: (x.attachmentIds ?? []).filter((aid) => aid !== attachmentId),
