...
export function ScheduledTasksPanel({
  ...
  const submit = () => {
    ...
    if (kind === "semana" && weekdays.length === 0) {
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="right" className="w-full sm:max-w-md overflow-y-auto">
        <SheetHeader>
          <SheetTitle>Tarefas agendadas</SheetTitle>
        </SheetHeader>
        <div className="space-y-4 mt-4">
          <div>
            ...
            <Input
              ...
              placeholder="Ex: Felicitar aniversariantes"
          ...
          <div>
            ...
            <Textarea
              ...
              className="mt-1.5 rounded-xl bg-muted border-0 resize-none"
            />
          </div>
          ...
          <div>
            ...
            <RadioGroup
              value={kind}
              onValueChange={(v) => setKind(v as ScheduledKind)}
              className="mt-2 space-y-2"
            >
              <label className="flex items-center gap-2 text-sm">
                <RadioGroupItem value="data" /> Data específica (avulsa)
              </label>
              <label className="flex items-center gap-2 text-sm">
                <RadioGroupItem value="semana" /> Dias da semana
              </label>
              <label className="flex items-center gap-2 text-sm">
                <RadioGroupItem value="recorrente" /> Recorrência avançada
              </label>
            </RadioGroup>
          </div>
            ...
            <div className="space-y-2 rounded-xl bg-muted/40 p-3">
              ...
              >
                {WEEKDAY_LABELS.map((l, i) => (
                  <ToggleGroupItem key={i} value={String(i)} className="h-9 w-9 rounded-lg">
                    {l}
                  </ToggleGroupItem>
                ))}
              </ToggleGroup>
                ...
                {scheduled.map((s) => (
                  <li
                    ...
                    className={cn(
                      ...
                      editingId === s.id && "ring-1 ring-brand/40",
                    )}
                  >
                    <div className="flex items-start gap-2">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          <Badge
                            variant="outline"
                            className="bg-muted border-0 text-[10px] shrink-0"
                          >
                            {s.kind === "data"
                              ? "Data"
                              : s.kind === "semana"
    ...
    <Popover>
      <PopoverTrigger asChild>
        <Button
          variant="outline"
          className={cn(
            "w-full justify-start rounded-xl bg-muted border-0 font-normal",
...
function describeSchedule(s: ReturnType<typeof useTaskStore>["scheduled"][number]) {
  ...
  if (s.kind === "semana" && s.weekdays)
    return `Dias: ${s.weekdays.map((d) => WEEKDAY_LABELS[d]).join(", ")}`;
