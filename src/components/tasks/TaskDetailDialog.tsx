          ...
          <section className="flex flex-col overflow-hidden min-h-0 border-l border-border pl-4">
                ...
                return (
                  <div className="mb-3 rounded-xl border border-border bg-muted/40 p-2 space-y-2">
                    <p className="text-[11px] font-semibold text-muted-foreground flex items-center gap-1">
                      <Pin className="h-3 w-3" /> Fixadas ({pinnedComments.length}/
                      {MAX_PINNED_COMMENTS})
                    </p>
                    {visiblePinned.map((c) => {
                      const atts = (c.attachmentIds ?? [])
                        ...
                        <TimelineRow key={`pin-${c.id}`} authorId={c.authorId} at={c.createdAt}>
                          <CommentBubble
                            ...
                            canEdit={
                              ...
                              Date.now() - new Date(c.createdAt).getTime() < EDIT_WINDOW_MS
                            }
                            canDelete={
                              c.authorId === currentUserId &&
                              Date.now() - new Date(c.createdAt).getTime() < EDIT_WINDOW_MS
                            ...
                            onRemoveAttachment={(aid) =>
                              removeCommentAttachment(task.id, c.id, aid)
                            }
                ...
                if (ev.kind === "created")
                  return (
                    <TimelineRow key={i} authorId={ev.by} at={ev.at}>
                      <em className="text-muted-foreground">criou a tarefa</em>
                    </TimelineRow>
                  );
                if (ev.kind === "moved")
                  return (
            ...
            >
              ...
              <div className="flex items-center justify-end gap-1 mt-1">
                ...
                <AudioRecorder
                  compact
                  onSend={(blob, sec) => addAudioMessage(task.id, blob, sec)}
                />
                <Button
                  onClick={submit}
                  disabled={!canSend}
