...
export function MentionInput({
  ...
  const mentionedNames = useMemo(() => {
    const scan =
      mentionStart != null ? value.slice(0, mentionStart) + value.slice(cursorPos) : value;
  ...
  const onKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    if (open && options.length) {
      ...
      if (e.key === "Enter" && !e.shiftKey) {
        e.preventDefault();
        insertMention(options[hoverIndex].name);
        return;
      }
      if (e.key === "Escape") {
        setOpen(false);
