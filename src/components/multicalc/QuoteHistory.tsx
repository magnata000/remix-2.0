...
export function QuoteHistory({
  ...
  useEffect(() => {
    if (focusedGroupId) {
      ...
      const t = setTimeout(() => {
        document
          .getElementById(`quote-group-${focusedGroupId}`)
    ...
    <div className="space-y-4">
        ...
        <div className="flex flex-col md:flex-row gap-2 md:items-center">
          ...
          >
            <span className="relative inline-flex mr-2">
              ...
              <Plus
                className="h-2.5 w-2.5 absolute -top-1 -left-1.5 bg-brand rounded-full p-px"
                strokeWidth={3}
              />
            </span>
            Nova cotação
      ...
      <div className="space-y-3">
        {filtered.map((g) => {
          ...
          return (
            <Card
              id={`quote-group-${g.groupId}`}
              key={g.groupId}
              className="rounded-2xl border-border shadow-none overflow-hidden scroll-mt-20"
            >
              <Collapsible
                open={isOpen}
                onOpenChange={(o) => setOpenGroups((s) => ({ ...s, [g.groupId]: o }))}
              >
                  ...
                  <button className="w-full p-4 flex items-center gap-3 hover:bg-muted/40 transition text-left">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <Badge variant="outline" className="bg-muted border-0">
                          {g.versions.length} {g.versions.length === 1 ? "versão" : "versões"}
                        </Badge>
                      ...
                      <p className="text-xs text-muted-foreground mt-1">
                        Última atualização em {formatDateShort(g.latest.createdAt)} · melhor preço{" "}
                        {formatBRL(winnerPrice)}
                      </p>
                    </div>
                    <div
                    ...
                    >
                      {g.status !== "ganha" && (
                        <Button
                          ...
                          onClick={() => handleStatus(g.groupId, "ganha")}
                        >
                      ...
                      {g.status !== "perdida" && (
                        <Button
                          ...
                          variant="outline"
                          className="rounded-lg"
                          onClick={() => handleStatus(g.groupId, "perdida")}
                        >
                          Perdida
                        </Button>
                      )}
                  ...
                  <div className="px-4 pb-4 border-t border-border">
                    <ol className="mt-3 space-y-2">
                      {g.versions.map((v, idx) => {
                        ...
                        const branchBlocked =
                          !isSelected && !!allowedBranch && allowedBranch !== v.branch;
                        const eff = effectiveStatus(v, g.versions);
                        return (
                          <li
                            key={v.id}
                            className="rounded-xl bg-muted/40 p-3 flex flex-col md:flex-row md:items-center gap-3"
                          >
                            <div className="flex items-start gap-3 flex-1 min-w-0">
                              <Checkbox
                                ...
                                title={
                                  branchBlocked
                                    ? "Só é possível comparar cotações do mesmo ramo"
                                    : undefined
                                }
                              />
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex flex-wrap items-center gap-2">
                                  ...
                                  <Badge className="bg-brand text-brand-foreground border-0 gap-1">
                                    <Trophy className="h-3 w-3" />
                                    {formatBRL(versionWinner?.price ?? 0)}
                                  </Badge>
                                  {idx === g.versions.length - 1 && eff !== g.status && (
                                ...
                                {diffs.length > 0 && (
                                  <div className="mt-2 flex flex-wrap gap-1">
                                    {diffs.map((d) => (
                                      <span
                                        key={d.field}
                                        className="inline-flex items-center gap-1 text-[11px] bg-warning/15 text-warning-foreground px-2 py-0.5 rounded-md"
                                      >
                                        {d.label}: <s className="opacity-60">{d.from || "—"}</s> →{" "}
                                        <strong>{d.to || "—"}</strong>
                                      </span>
                                    ))}
                                  </div>
