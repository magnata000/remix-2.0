  ...
  const previstaVsReal = useMemo(() => {
    return cashFlow.map((_, idx) => {
      ...
      const prevista = commissions
        .filter(
          (c) => c.status !== "cancelada" && c.status !== "devolvido" && inRange(c.dueDate, r),
        )
        .reduce((s, c) => s + c.amount, 0);
    ...
    <div className="space-y-5">
            ...
            <div className="space-y-0.5">
              <div>
                ...
                <span className="tabular-nums font-medium text-foreground">
                  {formatBRL(projected.saldoAtual)}
                </span>
              </div>
              <div>
                + Recebimentos:{" "}
                <span className="tabular-nums font-medium text-success">
                  {formatBRL(projected.recebimentos)}
                </span>
              </div>
              <div>
                − Pagamentos:{" "}
                <span className="tabular-nums font-medium text-destructive">
                  {formatBRL(projected.pagamentos)}
                </span>
              </div>
            </div>
          }
      ...
      <Card className="p-5 rounded-2xl border-border shadow-none">
        <div className="mb-4">
          ...
          <p className="text-xs text-muted-foreground">
            Prevista considera comissões contratadas por vencimento; realizada, valores efetivamente
            recebidos.
          </p>
        </div>
        <div className="h-72">
          {previstaVsReal.length === 0 ? (
            <EmptyState />
          ) : (
            <ResponsiveContainer width="100%" height="100%">
              <LineChart
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <Card className="p-5 rounded-2xl border-border shadow-none">
          ...
          <div className="h-64">
            {pieData.length === 0 ? (
              <EmptyState label="Nenhuma saída no período." />
            ) : (
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
      ...
      <Card className="p-5 rounded-2xl border-border shadow-none">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
          <KpiCard
            ...
            value={formatBRL(del.valorEmAberto)}
            hint="Soma das comissões com status Atrasado."
          />
          <KpiCard
            title="Inadimplência (%)"
            value={formatPct(del.pct)}
            hint="Valor em aberto ÷ (Atrasado + Pendente)."
            invertTrendColor
          />
          <KpiCard
            title="Parcelas em Atraso"
            value={String(del.parcelas)}
            hint="Número de comissões atrasadas."
          />
          <KpiCard
            title="Clientes Inadimplentes"
            value={String(del.clientes)}
            hint="Clientes distintos com pelo menos uma parcela atrasada."
          />
        </div>
        <div className="h-64">
          {del.serieMensal.every((s) => s.value === 0) ? (
            <EmptyState label="Sem histórico de inadimplência." />
          ) : (
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
                data={del.serieMensal}
                margin={{ top: 10, right: 10, left: -20, bottom: 0 }}
              >
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="var(--border)" />
                <XAxis dataKey="month" tickLine={false} axisLine={false} fontSize={12} />
