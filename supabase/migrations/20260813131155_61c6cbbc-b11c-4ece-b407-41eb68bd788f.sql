...
CREATE INDEX idx_follow_ups_date   ON public.follow_ups(date);
...
CREATE TABLE public.expenses (
  ...
  dre_kind    public.dre_category_kind NOT NULL DEFAULT 'outra',
...
CREATE POLICY "Admin manage members" ON public.team_members
...
CREATE POLICY "Read via policy" ON public.commissions
  FOR SELECT TO authenticated
  USING (EXISTS (SELECT 1 FROM public.policies p WHERE p.id = commissions.policy_id
    AND (p.assignee_id = auth.uid() OR public.has_role(auth.uid(), 'admin'))));
CREATE POLICY "Write via policy" ON public.commissions
  FOR ALL TO authenticated
  USING (EXISTS (SELECT 1 FROM public.policies p WHERE p.id = commissions.policy_id
    AND (p.assignee_id = auth.uid() OR public.has_role(auth.uid(), 'admin'))))
  WITH CHECK (EXISTS (SELECT 1 FROM public.policies p WHERE p.id = commissions.policy_id
    AND (p.assignee_id = auth.uid() OR public.has_role(auth.uid(), 'admin'))));

CREATE POLICY "Read via client" ON public.follow_ups
  FOR SELECT TO authenticated
  USING (EXISTS (SELECT 1 FROM public.clients c WHERE c.id = follow_ups.client_id
    AND (c.assignee_id = auth.uid() OR public.has_role(auth.uid(), 'admin'))));
CREATE POLICY "Write via client" ON public.follow_ups
  FOR ALL TO authenticated
  USING (EXISTS (SELECT 1 FROM public.clients c WHERE c.id = follow_ups.client_id
    AND (c.assignee_id = auth.uid() OR public.has_role(auth.uid(), 'admin'))))
  WITH CHECK (EXISTS (SELECT 1 FROM public.clients c WHERE c.id = follow_ups.client_id
    AND (c.assignee_id = auth.uid() OR public.has_role(auth.uid(), 'admin'))));
...
CREATE POLICY "Read via client" ON public.doc_folders
  FOR SELECT TO authenticated
  USING (EXISTS (SELECT 1 FROM public.clients c WHERE c.id = doc_folders.client_id
    AND (c.assignee_id = auth.uid() OR public.has_role(auth.uid(), 'admin'))));
CREATE POLICY "Write via client" ON public.doc_folders
  FOR ALL TO authenticated
  USING (EXISTS (SELECT 1 FROM public.clients c WHERE c.id = doc_folders.client_id
    AND (c.assignee_id = auth.uid() OR public.has_role(auth.uid(), 'admin'))))
  WITH CHECK (EXISTS (SELECT 1 FROM public.clients c WHERE c.id = doc_folders.client_id
    AND (c.assignee_id = auth.uid() OR public.has_role(auth.uid(), 'admin'))));

CREATE POLICY "Read via client" ON public.doc_files
  FOR SELECT TO authenticated
  USING (EXISTS (SELECT 1 FROM public.clients c WHERE c.id = doc_files.client_id
    AND (c.assignee_id = auth.uid() OR public.has_role(auth.uid(), 'admin'))));
CREATE POLICY "Write via client" ON public.doc_files
  FOR ALL TO authenticated
  USING (EXISTS (SELECT 1 FROM public.clients c WHERE c.id = doc_files.client_id
    AND (c.assignee_id = auth.uid() OR public.has_role(auth.uid(), 'admin'))))
  WITH CHECK (EXISTS (SELECT 1 FROM public.clients c WHERE c.id = doc_files.client_id
    AND (c.assignee_id = auth.uid() OR public.has_role(auth.uid(), 'admin'))));